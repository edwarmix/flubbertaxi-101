...
<body dir="{{(setting('language_rtl', false) ? 'rtl' : 'ltr')}}">
    <nav class="navbar navbar-expand-lg navbar-light bg-light">
        <a class="navbar-brand" href="{{ route('home') }}"><img src="{{ $app_logo }}"
                title="{{ setting('app_name') }}" height="50"> {{ setting('app_name') }}</a>
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="navbar-nav mr-auto">
                <li class="nav-item {{ \Illuminate\Support\Facades\Route::is('home') ? 'active' : '' }}">
                    <a class="nav-link" href="{{ route('home') }}">{{ __('Place an Ride') }}</a>
                </li>
                <li class="nav-item {{ \Illuminate\Support\Facades\Route::is('rides.index') ? 'active' : '' }}">
                    <a class="nav-link" href="{{ route('rides.index') }}">{{ __('My Rides') }}</a>
                </li>
                @can('admin.dashboard')
                ...
                @if (!auth()->check())
                    ...
                    &nbsp;
                    <button class="btn btn-outline-success createAccount"
                        id="">{{ __('Create an Account') }}</button>
                @else
                    <div class="nav-item dropdown">
                        ...
                        <div class="dropdown-menu" aria-labelledby="navbarDropdown">
                            <a class="dropdown-item" href="{{ route('home') }}">{{ __('Place an Ride') }}</a>
                            @can('admin.dashboard')
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header text-center">
                            <h4 class="modal-title" id="modalTitleLogin">{{ __('You need to login to make rides') }}</h4>
                        </div>
                        <div class="modal-body">
                                ...
                                <form method="POST" action="{{ route('login') }}" onsubmit="return false;">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input aria-describedby="emailHelpBlock" id="email" type="email"
                                            ...
                                            name="email" placeholder="{{ __('Input your email') }}" tabindex="1"
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input aria-describedby="passwordHelpBlock" id="password" type="password"
                                            value="" placeholder="{{ __('Input your password') }}"
                                            ...
                                            @foreach (['facebook', 'google', 'twitter', 'linkedin', 'github', 'gitlab', 'bitbucket'] as $social)
                                                @if (setting('enable_' . $social, false))
                                                    <a href="{{ url('login/' . $social) }}" class="social-button"
                                                        id="{{ $social . '-connect' }}">
                                                        <i class="fab fa-{{ $social }} mr-3"></i>
                                                        {{ __('auth.login_' . $social) }}
                                                    </a>
                                                @endif
                                            @endforeach
                                        </div>
                                    ...
                                    <div class="mt-5 text-muted text-center">
                                        {{ __('Don\'t have an account?') }} <a href="#"
                                            class="btn btn-block btn-success createAccount">{{ __('Create account') }}</a>
                                    </div>
                                </form>
                            </div>
                            <div id="createAccountContent" style="display: none">
                                <form method="POST" action="#" onsubmit="return false;">
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <input id="firstName" type="text"
                                                    ...
                                                    name="name" placeholder="{{ __('Input your name') }}"
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <input id="password" type="password"
                                                    class="form-control{{ $errors->has('password') ? ' is-invalid' : '' }}"
                                                    placeholder="{{ __('Input your password') }}" name="password"
                                                    required>
                                                <div class="invalid-feedback">
                                        ...
                                        <div class="col-md-12 mt-4">
                                            @if (setting('enable_terms_of_service') || setting('enable_privacy_policy'))
                                                <div class="form-group">
                                                    ...
                                                    @if (setting('enable_terms_of_service'))
                                                        <a href="{{ route('terms') }}"
                                                            target="_blank">{{ __('Terms of Use') }}</a>
                                                    @endif
                                                    @if (setting('enable_terms_of_service') && setting('enable_privacy_policy'))
                                                        {{ ' ' . __('and') . ' ' }}
                                                    @endif
                                            ...
                                            @endif
                                            <button type="button"
                                                class="btn btn-primary btn-lg btn-block btnCreateAccount">
                                                {{ __('Create Account') }}
                                            </button>
                                            @if (setting('enable_facebook', false) ||
                                                setting('enable_google', false) ||
                                    ...
                                    <div class="mt-5 text-muted text-center">
                                        {{ __('Already have an account?') }} <a href="#"
                                            class="loginAccount">{{ __('Log in') }}</a>
                                    </div>
                                </form>
                            </div>
                        </div>
        ...
        <link rel="stylesheet"
            href="https://cdn.jsdelivr.net/npm/@sweetalert2/theme-bootstrap-4@5.0.6/bootstrap-4.min.css">
                ...
                $('.btnLogin').click(function() {
                    ...
                    $.ajax({
                        ...
                        error: function(data) {
                            ...
                            Swal.fire({
                                text: data.message ??
                                    '{{ __('An error occurred doing your login, try again later.') }}',
                                icon: 'error',
                                type: 'error',
                                showCancelButton: false,
                                confirmButtonColor: '#3085d6',
                ...
                $('.btnCreateAccount').click(function() {
                    ...
                    $.ajax({
                        ...
                        error: function(data) {
                            ...
                            Swal.fire({
                                text: data.message ??
                                    '{{ __('An error occurred while creating your account, try again later.') }}',
                                icon: 'error',
                                type: 'error',
                                showCancelButton: false,
                                confirmButtonColor: '#3085d6',
