 ...
 * Show the value formated to user in browser
 */
function getDriverStatusColumn($value)
{
    switch ($value) {
        case 'approved':
            return '<span class="badge badge-success">' . __('Approved') . '</span>';
        case 'reproved':
            return '<span class="badge badge-danger">' . __('Reproved') . '</span>';
        default:
 ...
 * Show the value formated to user in browser
 */
function getAvailableDriverStatus()
{
    return [
        'approved' => __('Approved'),
        'pending' => __('Pending'),
        'reproved' => __('Reproved'),
    ];
}
...
function getDateHumanFormat(\Carbon\Carbon|string $date, $htmlTooltip = false)
{
    ...
    if ($htmlTooltip) {
        return '<p data-toggle="tooltip" data-placement="bottom" title="' . $date->format(getLocaleDateFormat(true)) . '">' . $date->diffForHumans() . '</p>';
    } else {
...
function getTextForRideCount($count)
{
    if ($count < 10) {
        return __('New!');
    } elseif ($count < 20) {
        return __(':quantity completed rides', ['quantity' => '+10']);
    } elseif ($count < 50) {
        return __(':quantity completed rides', ['quantity' => '+20']);
    } elseif ($count < 100) {
        return __(':quantity completed rides', ['quantity' => '+50']);
    } elseif ($count < 1000) {
        return __(':quantity completed rides', ['quantity' => '+100']);
    } else {
        return __(':quantity completed rides', ['quantity' => '+1000']);
    }
}
...
function getAvailablePaymentGatewaysArray()
{
    ...
    if (setting('enable_razorpay')) {
        $array['razorpay'] = 'RazorPay';
    }
    if (setting('enable_flutterwave')) {
        $array['flutterwave'] = 'Flutterwave';
    }
    return $array;
}

/*
 * Return the ride status options in array
 */
function getAvailableRideStatusArray()
{
    return [
        'waiting' => trans('general.ride_status_list.waiting'),
        'pending' => trans('general.ride_status_list.pending'),
        'accepted' => trans('general.ride_status_list.accepted'),
        'in_progress' => trans('general.ride_status_list.in_progress'),
        'completed' => trans('general.ride_status_list.completed'),
        'cancelled' => trans('general.ride_status_list.cancelled'),
    ];
}
...
function getAvailablePaymentStatusArray()
{
    return [
        'pending' => trans('general.ride_status_list.pending'),
        'paid' => trans('general.ride_status_list.paid'),
        'cancelled' => trans('general.ride_status_list.cancelled'),
    ];
}
