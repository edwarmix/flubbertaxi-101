...
@section('content')
    <section class="section">
        ...
        <div class="section-body">
            <div class="row">
                <div class="col-md-8">
                    <h6>Rides Revenue Chart</h6>
                    <canvas id="myChart"></canvas>
                </div>
                <div class="col-md-4">
                    <div class="card card-info card-body">
                        <h3><i class="fa fa-clock"></i> {{ $ridesInProgressCount }}</h3>
                        <p>{{ __('Rides in Progress') }}</p>
                    </div>
                    <div class="card card-warning card-body">
                        <h3><i class="fa fa-motorcycle"></i> {{ $activeDriversCount }}</h3>
                        <p>{{ __('Active Drivers') }}</p>
                    </div>
...
` + ((item.only_recent) ? '{{ __('Only items from the last 24h are displayed in this list') }}' :
                        '{{ __('All items in this status are displayed in this list') }}') + `</small>
</h3>
                    </div>
            ...
            function getRides() {
                $('.dropzone').each(function() {
                    ...
                    console.log(status);
                    $.ajax({
                        url: "{{ route('admin.dashboard.ajaxGetRides') }}",
                        type: "POST",
                        dataType: "json",
                        data: {
                            "status": status,
            ...
            function getRideCard(card, status_color) {
                return `
            <div id="` + card.id + `" class="kanbanCard" style="border-left: 4px solid ` + status_color + `;">
                <div class="content">
                        ...
                        <thead>
                            <tr>
                                ...
                                <th class="description">
                                    ...
                                    <i class="fas fa-motorcycle"></i> ` + card.driver_name + `<br>
                                    <i class="fas fa-user"></i> ` + card.customer_name + `<br>
                                    <i class="fas fa-home"></i> ` + card.boarding_location + `<br>
                                    <i class="fas fa-map"></i> ` + card.ride_location + `<br>
                                    ` + (card.return_location ?
                    '<i class="fas fa-check"></i> {{ __('Return to pickup location') }}<br>' : '') + `
                    ...
                    <div class="orderPrices" style="padding: 5px 5px;">
                        {{ trans('Driver Value') }}: ` + card.app_value + `<br>
                        {{ trans('App Fee') }}: ` + card.driver_value + `<br>
                        {{ trans('Total') }}: ` + card.total + `
                    </div>
                    <div class="orderActions">
                        <a href="` + card.link + `" class='btn btn-block btn-outline-primary btn-sm' target="_blank">{{ __('Open Ride') }} <i class="fas fa-external-link"></i></a>
                    </div>
                </div>
            </div>
            `;
                $(`.dropzone[data-item="` + card.ride_status + `"]`).append(htmlString);
